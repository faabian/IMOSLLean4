...
-- theorem prod_map_succ_iota_aux : ∀ n : ℕ, (Ico 1 (n+1)).prod = n.factorial
--   | 0 => rfl
--   | n + 1 => by
--     rw [List.Ico.succ_top]
--     simp [prod_map_succ_iota_aux n, Nat.factorial_succ, mul_comm]
--     exact Nat.le_add_left 1 n

-- theorem prod_map_succ_iota (n : ℕ) : ((Ico 1 (n+1)).map (1).add).prod = (n+1).factorial := by
--   have := prod_map_succ_iota_aux (n+1)
--   rw [← this]
--   have h : map (1).add (Ico 1 (n + 1)) = Ico 2 (n+2) := List.Ico.map_add 1 (n+1) 1
--   rw [h]
--   simp [List.Ico.eq_cons]


-- theorem targetSum_general_lower_bound :
--     ∀ l : List ℕ, (l.map (1).add).prod ≤ 2 ^ targetSum l * l.length.factorial
--   | [] => Nat.le_refl 1
--   | a :: l => by
--       rw [map_cons, prod_cons, targetSum, pow_add,
--         length_cons, Nat.factorial_succ, mul_mul_mul_comm]
--       have := Nat.mul_le_mul (succ_le_mul_two_pow_div a l.length.succ_pos)
--         (targetSum_general_lower_bound l)
--       rwa [add_comm] at this

-- theorem targetSum_perm_iota_n_lower_bound (hn : n ≠ 0) (h : l ~ Ico 1 (n+1)) : Nat.log2 n < targetSum l := by
--   have h0 := targetSum_general_lower_bound l
--   have := h.map (1).add
--   rw [this.prod_eq] at h0
--   rw [prod_map_succ_iota] at h0
--   simp [h.length_eq, Nat.factorial_succ] at h0
--   rw [Nat.log2_lt hn]
--   have := Nat.le_of_mul_le_mul_right h0 (Nat.factorial_pos _)
--   exact this
...
--     | false, 0 => l
--     | false, k + 1 => (Ico 1 (k+1)).map (k + 1).add ++ (2 * k + 2) :: l
--     | true, n => (Ico 1 (n+1)).map n.add ++ (2 * n + 1) :: l
...
--       = (Ico 1 (k+1)).map (k + 1).add ++ (2 * k + 2) :: lowerBoundMk (k + 1) :=
--   Nat.binaryRec_eq _ _ (by simp)

-- lemma lowerBoundMk_bit1 (n : ℕ) :
--     lowerBoundMk (n.bit true) = (Ico 1 (n+1)).map n.add ++ (2 * n + 1) :: lowerBoundMk n :=
--   Nat.binaryRec_eq _ _ (by simp)
...
-- theorem lowerBoundMk_perm_iota : ∀ n : ℕ, lowerBoundMk n ~ (Ico 1 (n+1)).map Nat.succ :=
...
--   (lowerBoundMk_perm_iota n).length_eq.trans ((length_map Nat.succ).trans length_range)

-- lemma targetSum_map_add_iota_length_succ (h : l.length = Nat.succ n) :
--     ∀ k, targetSum ((range k).map (n+1).add ++ l) = targetSum l
--   | 0 => rfl
--   | k + 1 => by
--       simp [range_cons, map_append, targetSum, length_cons,
